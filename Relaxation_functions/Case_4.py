...
for i in range(n_learn):
    ...
    if i > l_a-1:
        ...
        omega_1 = (beta-1)*math.exp(-alpha*i)+1
        omega_2 = (1-beta)*math.exp(-alpha*i)
    for k in range(n_end-1): # Collect Data
        ...
        Psi[k] = omega_1*( x[:, k].T @ Q @ x[:, k] + u[:, k].T * R * u[:, k] - gamma**2 * w[:, k].T * w[:, k]+x[:, k+1].T @ P[i] @ x[:, k+1]) + omega_2*x[:, k].T @ P[i] @ x[:, k]
